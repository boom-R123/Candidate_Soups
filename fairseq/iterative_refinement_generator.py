...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        for step in range(self.max_iter + 1):
            ...
            decoder_out = model.forward_decoder(
                prev_decoder_out, encoder_out, reranker=reranker, encoder_input=[src_tokens, src_lengths], batch=batch_size, Use_CDS=Use_CDS, **decoder_options
            )
        ...
        if self.beam_size > 1:
            if reranker is not None and not Use_CDS:
    ...
    def rerank(self, reranker, finalized, encoder_input, beam_size):
        ...
        length_beam_order = (
            utils.new_arange(
                final_output_tokens, beam_size, reranker_encoder_out["encoder_out"][0].size(1)
            )
            .t()
            .reshape(-1)
        )
